...
Отличия в разамерах с предыдущими показателями вызваны тем, что эти измерения показаны для другого теста, где tps был сильно выше wal файлов были сильно больше.

На первый взгляд checkpoint затрачивалось около 56,5 секунд. Это видно как из запроса `select * from pg_stat_bgwriter;`
...
Почему так произошло не понял. Поэтому Проверяем все параметры в соответсвии с этой статьёй habr и документацией postgres, но не нашёл в чём причина. Скорее у меня подозрение, что из этих 57 требуется вычитать 30(чутьё). 
...
Видим, что асинхронаая запись получилась где-то в 3-7 раз быстрее, чем синхронная. Почему: по всей видимсоти pgbench даёт OLTP нагрузку и существеннове время съедает подтверждение записи тарнзакций на диск. При асинхронной работе мы на эту активность время не тратим, транзакция считается завершённой сразу после логического завершения ещё до записи на диск -> более высокая производительность.
...
Очевдно, что checksum не сходится. Для того, точбы вытащить данные выполняем
