...
P.S В этой [статье](https://www.percona.com/blog/tuning-postgresql-for-sysbench-tpcc/) сказано, что включение компрессии уменьшает нагрузку т.к. уменьшает количество и частоту записи WAL, но мой опыт говорит о том, что это такое себе, либо я чего-то не понимаю. Даже если учесть, что в статье упомминается избыток CPU. Я уверен, что если нагрузка реально высока, то такая настройка приведёт к "тормозам".ZZ
